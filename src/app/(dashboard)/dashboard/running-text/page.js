...
import { LOGO_DATA, buildLogoSrc, buildOtherLogoSrc, resolveAnyClubLogo } from '@/lib/logoData'
...
export default function RunningTextOverlay() {
  ...
  if (!tickerItems.length)
    return <div className='text-white p-2 font-mono text-xs'>No active or recent matches found.</div>

  if (!mounted) return null // Avoid hydration mismatch

  const { y, scale } = settings?.runningText || { y: 0, scale: 1 }

  // Safety check for invalid values that could crash style prop
  ...
  return (
    <div
      className='fixed inset-0 overflow-hidden flex flex-col justify-end bg-transparent pb-8'
      style={{
