...
export default function LayoutD({ data, displayTime, formatTime }) {
  ...
  const teamLeftColor = isSwapped
    ? data.awayColor || data.awayBg || '#0df2b9'
    : data.homeColor || data.homeBg || '#ff5500'
        ...
        <div className='live-hud'>
          ...
          <div className='hud-fnc-panel'>
            <div className='hud-logo-container'>
              <img src={teamLeftLogo} alt={teamLeftName} className='hud-team-logo' />
            </div>
          ...
          <div className='hud-score-container-home'>
            <div
              className={`hud-fnc-score ${showGoal && goalTeam === (isSwapped ? data.awayName : data.homeName) ? 'goal-active' : ''}`}
            >
            ...
            {(data.seriesType === 'bo3' || data.seriesType === 'bo5') && (
              <div className='hud-timeout-tab'>
                {Array.from({ length: data.seriesType === 'bo3' ? 2 : 3 }).map((_, i) => (
                  <div key={i} className={`hud-diamond ${i < teamLeftSeriesScore ? 'active' : ''}`} />
                ))}
              </div>
            )}
          </div>
          ...
          <div className='hud-score-container-away'>
            <div
              className={`hud-nrg-score ${showGoal && goalTeam === (isSwapped ? data.homeName : data.awayName) ? 'goal-active' : ''}`}
            >
            ...
            {(data.seriesType === 'bo3' || data.seriesType === 'bo5') && (
              <div className='hud-timeout-tab'>
                {Array.from({ length: data.seriesType === 'bo3' ? 2 : 3 }).map((_, i) => (
                  <div key={i} className={`hud-diamond ${i < teamRightSeriesScore ? 'active' : ''}`} />
                ))}
              </div>
            )}
          </div>
          ...
          <div className='hud-nrg-panel'>
            ...
            <div className='hud-logo-container-right'>
              <img src={teamRightLogo} alt={teamRightName} className='hud-team-logo' />
            </div>
          </div>
        </div>
      </div>
