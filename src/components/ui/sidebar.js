...
export function SidebarItem({ active = false, isDark = false, className = '', ...props }) {
  const base = 'dashboard-nav-item'

  const activeClass = isDark ? 'dashboard-nav-item-active-dark' : 'dashboard-nav-item-active-light'

  const inactiveClass = isDark ? 'dashboard-nav-item-inactive-dark' : 'dashboard-nav-item-inactive-light'
