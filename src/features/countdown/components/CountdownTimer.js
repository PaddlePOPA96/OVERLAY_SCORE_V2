  ...
  return (
    ...
    >
      ...
      <div className='flex flex-col items-center justify-center mb-8'>
        ...
        >
          <div className='flex flex-col items-center'>
            ...
            <span
              className={`text-xs sm:text-sm md:text-base uppercase tracking-widest font-semibold mt-2 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}
            >
              Days
            </span>
          </div>
        ...
        <div className='flex gap-4 mt-6'>
          {!isRunning && currentRemaining > 0 ? (
            <button
              ...
              className='px-8 py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold transition-all shadow-lg hover:shadow-green-600/30'
            >
              Resume
            </button>
          ) : isRunning ? (
            <button
              onClick={handlePause}
              className='px-8 py-3 bg-yellow-500 hover:bg-yellow-600 text-white rounded-xl font-bold transition-all shadow-lg hover:shadow-yellow-500/30'
            >
              Pause
            </button>
          ) : null}
          <button
            onClick={handleReset}
            className={`px-8 py-3 rounded-xl font-bold transition-all border ${isDark ? 'border-slate-700 text-slate-300 hover:bg-slate-800' : 'border-slate-300 text-slate-700 hover:bg-slate-100'}`}
          >
            Reset
          </button>
        </div>
      ...
      <div className='grid md:grid-cols-2 gap-8 mt-8'>
        {/* Presets */}
        <div
          className={`p-6 rounded-xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200'}`}
        >
          <h3 className={`text-lg font-bold mb-4 ${isDark ? 'text-white' : 'text-slate-900'}`}>Quick Presets</h3>
          <div className='grid grid-cols-1 sm:grid-cols-3 gap-3'>
            <button
              onClick={() => handleStartPreset(30 * 60 * 1000)}
              className='px-4 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-semibold transition-all'
            >
              30 Mins
            </button>
            <button
              onClick={() => handleStartPreset(60 * 60 * 1000)}
              className='px-4 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-semibold transition-all'
            >
              60 Mins
            </button>
            <button
              onClick={() => handleStartPreset(24 * 60 * 60 * 1000)}
              className='px-4 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-semibold transition-all'
            >
              1 Day
            </button>
        ...
        >
          ...
          <div className='flex gap-2 mb-4'>
            <div className='flex-1'>
              ...
              <input
                ...
                className={`w-full p-2 rounded-lg border text-center ${isDark ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300 text-slate-900'}`}
                placeholder='0'
              />
            </div>
            <div className='flex-1'>
          ...
          <button
            ...
            className='w-full px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold transition-all shadow-lg hover:shadow-blue-600/30'
          >
            Start Manual Timer
          </button>
        </div>
        ...
        >
          ...
          <div className='mb-6'>
            ...
            <div className='flex gap-2 items-center'>
              <span
                className={`flex-1 p-3 rounded-lg border font-mono text-sm ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-300'}`}
              >
          ...
          <h3
            className={`text-lg font-bold mb-4 border-t pt-4 ${isDark ? 'border-slate-700/50 text-white' : 'border-slate-200 text-slate-900'}`}
          >
            OBS Setup
          </h3>
          <div>
            ...
            <div className='flex gap-2'>
              <code
                className={`flex-1 p-3 rounded text-sm font-mono overflow-x-auto whitespace-nowrap ${isDark ? 'bg-black text-green-400' : 'bg-slate-200 text-slate-800'}`}
              >
      ...
      {isSoundModalOpen && (
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4'>
          <div
            className={`w-full max-w-md p-6 rounded-2xl shadow-2xl ${isDark ? 'bg-slate-900 border border-slate-800' : 'bg-white border border-slate-200'}`}
          >
