...
export function PremierLeagueRight({ matches, loading, news, loadingNews, theme }) {
  const isDark = theme === 'dark'

  const headingClass = isDark ? 'text-gray-400' : 'text-slate-700'
  ...
  const liveMatch = matches?.find(m => m.status === 'IN_PLAY' || m.status === 'PAUSED') || null
  ...
  if (liveMatch) {
    if (liveMatch.status === 'FINISHED') {
      scoreText = `${liveMatch.score.fullTime.home ?? 0} : ${liveMatch.score.fullTime.away ?? 0}`
      statusLabel = 'FT'
    } else if (liveMatch.status === 'IN_PLAY' || liveMatch.status === 'PAUSED') {
      scoreText = `${liveMatch.score.fullTime.home ?? '-'} : ${liveMatch.score.fullTime.away ?? '-'}`
      statusLabel = 'Live'
    } else {
      scoreText = 'VS'
      statusLabel = 'Preview'
  ...
  return (
    <>
      <div>
        <div className={`${headingClass} mb-4 text-sm font-semibold tracking-wide`}>LIVE MATCHES</div>
          ...
          <div className='flex justify-between items-center mb-6 px-2'>
            <div className='flex flex-col items-center gap-2'>
              ...
              <span className='text-xs font-bold text-gray-300'>
                {liveMatch?.homeTeam?.shortName || liveMatch?.homeTeam?.name || 'Home'}
              </span>
            </div>
            <div className='flex flex-col'>
              <span className='text-3xl font-black text-white tracking-widest'>{scoreText}</span>
            ...
            <div className='flex flex-col items-center gap-2'>
              ...
              <span className='text-xs font-bold text-gray-300'>
                {liveMatch?.awayTeam?.shortName || liveMatch?.awayTeam?.name || 'Away'}
              </span>
            </div>
          </div>
          <button
      ...
      <div className='flex-1 overflow-hidden flex flex-col'>
        <div className={`${headingClass} mb-4 text-sm font-semibold tracking-wide`}>LATEST NEWS</div>
        <div className='space-y-4 overflow-y-auto pr-2'>
          {(loading || loadingNews) && (
            <p className='text-xs text-gray-400'>Mengambil informasi pertandingan dari API...</p>
          )}
          {!loadingNews && news && news.length > 0 ? (
            ...
            <p className='text-xs text-gray-400'>Tidak dapat memuat berita terkini.</p>
          ) : null}
        </div>
      </div>
...
function NewsItem({ title, date, image, isDark, onClick }) {
  ...
  return (
    <button type='button' onClick={onClick} className='flex gap-4 group cursor-pointer text-left w-full'>
      {image ? (
        <img
          src={image}
          alt={title}
      ...
      <div className='flex flex-col justify-center'>
        <h4
          className={`text-sm font-medium leading-snug mb-1 transition ${isDark ? 'text-white group-hover:text-purple-300' : 'text-black group-hover:text-purple-700'}`}
        >
          {title}
        </h4>
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/70'>
      <div className={`${containerClass} rounded-2xl p-4 w-full max-w-lg shadow-2xl`}>
        ...
        {item.image && (
          <div className='mb-3'>
            <img src={item.image} alt={item.title} className='w-full max-h-60 object-cover rounded-xl' />
          </div>
        )}
        {item.description && (
          <p className={`text-xs leading-relaxed mb-3 ${dark ? 'text-white' : 'text-black'}`}>{item.description}</p>
        )}
        {item.url && (
          <a
            href={item.url}
