        ...
        <div className='bg-gradient-to-r from-purple-800 to-indigo-900 rounded-2xl p-8 mb-8 relative overflow-hidden h-64 flex items-center shadow-lg shadow-purple-900/20 w-full'>
          <div className='w-2/3 z-10'>
            <div className='flex items-center gap-2 mb-2'>
              <span className='bg-white/20 px-2 py-0.5 rounded text-xs text-white'>Match of the Day</span>
              {isAdmin && (
                <button
                  type='button'
            ...
            <h2 className='text-3xl text-white font-bold mb-4'>
              {featured ? formatMatchTitle(featured) : 'Tidak ada pertandingan live'}
            </h2>
            <div className='space-y-1 text-sm text-gray-200 mb-6'>
              {featured ? (
                <>
                  <p className='flex items-center gap-2'>📅 {formatMatchVenue(featured)}</p>
                  <p className='flex items-center gap-2'>
                    🏟 {featured?.venue || featured?.competition?.name || 'Stadium informasi belum tersedia'}
                  </p>
                </>
              ) : (
                <p className='flex items-center gap-2'>
          ...
          <div className='space-y-8 w-full'>
            {loading ? (
              <div className={`${cardClass} rounded-xl p-8`}>
                <p className='text-center text-gray-400 text-sm'>Memuat jadwal dan hasil...</p>
              </div>
            ) : !matches || matches.length === 0 ? (
              <div className={`${cardClass} rounded-xl p-8`}>
                <p className='text-center text-gray-400 text-sm'>Tidak ada pertandingan yang tercatat.</p>
              </div>
            ) : (
              <>
                ...
                <div className={`${cardClass} rounded-xl p-5`}>
                  ...
                  <div className='space-y-2'>
                    {todayMatches.length > 0 ? (
                      todayMatches.map(m => <PremierLeagueMatchRow key={`today-${m.id}`} match={m} theme={theme} />)
                    ) : (
                      <p className='text-xs text-gray-400 italic'>Tidak ada pertandingan hari ini.</p>
                    )}
                  </div>
                </div>
                ...
                <div className={`${cardClass} rounded-xl p-5`}>
                  ...
                  <div className='space-y-2'>
                    {recentMatches.length > 0 ? (
                      recentMatches.map(m => <PremierLeagueMatchRow key={`last-${m.id}`} match={m} theme={theme} />)
                    ) : (
                ...
                <div className={`${cardClass} rounded-xl p-5`}>
                  ...
                  <div className='space-y-2'>
                    {nextMatches.length ? (
                      nextMatches.map(m => <PremierLeagueMatchRow key={`next-${m.id}`} match={m} theme={theme} />)
                    ) : (
...
function PremierLeagueTableCard({ standings, loadingStandings, cardClass, isDark, isAdmin, onRefreshStandings }) {
  ...
  const posClass = isDark ? 'py-2 px-2 text-white text-sm' : 'py-2 px-2 text-black text-sm'

  const clubClass = isDark ? 'py-2 pr-2 text-white font-medium text-sm' : 'py-2 pr-2 text-black font-medium text-sm'

  const playedClass = isDark ? 'py-2 px-2 text-center text-white text-sm' : 'py-2 px-2 text-center text-black text-sm'
  ...
  return (
    <div className={`${cardClass} rounded-xl p-5 space-y-5`}>
      ...
      {loadingStandings ? (
        <p className='text-center text-gray-400 text-sm'>Memuat klasemen Premier League...</p>
      ) : !standings || standings.length === 0 ? (
        <p className='text-center text-gray-400 text-sm'>Data klasemen tidak tersedia.</p>
      ) : (
        <>
          <div className='overflow-x-auto'>
            <table className='min-w-full text-sm leading-tight'>
              ...
              <tbody>
                {standings.map(row => (
                  <tr key={row.team?.id || `team-${row.position}`} className='border-b border-slate-100/40'>
                    <td className={posClass}>{row.position}</td>
                    <td className={clubClass}>
                      <div className='flex items-center gap-2'>
                        {(() => {
                          const logoSrc = resolveClubLogo(league, row.team?.name)
          ...
          <div className='pt-3 border-t border-slate-700/50 mt-3'>
            ...
            <ul className='mt-2 space-y-1'>
              {uclSpots.map(row => (
                <li key={`ucl-${row.team?.id || row.position}`} className='flex items-center justify-between'>
                  <div className='flex items-center gap-2'>
                  ...
                  <span className='text-[11px] font-mono text-emerald-400'>{row.points} pts</span>
                </li>
              ))}
            </ul>
          </div>
...
function resolveClubLogo(league, apiName) {
  ...
  if (!best) {
    best = clubs.find(club => normalizeTeamName(club).includes(target))
  }

  if (!best) {
    best = clubs.find(club => target.includes(normalizeTeamName(club)))
  }
...
export function PremierLeagueMatchRow({ match, theme }) {
  ...
  const minute = match.status === 'IN_PLAY' || match.status === 'PAUSED' ? 'LIVE' : isFinished ? 'FT' : 'UPCOMING'
  ...
  return (
    <div className='flex flex-col sm:flex-row items-center justify-between py-3 hover:bg-gray-700/50 rounded px-3 transition cursor-pointer group gap-3 sm:gap-4'>
      ...
      <div className='flex items-center justify-center sm:justify-start gap-3 sm:gap-4 w-full sm:flex-1 sm:min-w-0'>
        <span className={`text-[10px] sm:text-xs font-mono border px-1 rounded ${badgeClass}`}>{minute}</span>
          ...
          <div className='flex flex-col min-w-0 text-center sm:text-left'>
            <span
              className={`truncate text-sm sm:text-base ${
                isDark ? 'text-white' : 'text-black group-hover:text-purple-700'
              }`}
            >
              {match.homeTeam?.shortName || match.homeTeam?.name || 'TBD'}
            </span>
            {dateLabel && <span className='text-[10px] text-gray-400 truncate'>{dateLabel}</span>}
          </div>
        </div>
      </div>
      ...
      <div className='flex items-center justify-center sm:justify-end gap-3 sm:gap-4 w-full sm:flex-1 sm:min-w-0'>
        <div className='flex items-center gap-2 sm:gap-3 flex-1 min-w-0 justify-center sm:justify-end'>
          <span
            className={`truncate text-sm sm:text-base order-2 sm:order-1 text-right ${
              isDark ? 'text-white' : 'text-black group-hover:text-purple-700'
            }`}
          >
