...
export default function OverlayRoomControls({ showOverlay, toggleOverlay, roomId, compact }) {
  const [copied, setCopied] = useState(false)

  const displayRoomId = roomId && roomId.includes('_slot') ? roomId.split('_slot')[0] : roomId
  const overlayPath = `/dashboard/operator/overlay/${displayRoomId || 'default'}`
  ...
  return (
    <div className='op-room-row'>
      ...
      <span className={`op-tiny font-bold ${showOverlay ? 'text-green-500' : 'text-red-500'}`}>
