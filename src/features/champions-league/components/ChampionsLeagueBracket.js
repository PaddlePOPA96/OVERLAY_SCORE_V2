...
export function ChampionsLeagueBracket({ matches, loadingMatches, theme, isAdmin, onRefreshMatches }) {
  const isDark = theme === 'dark'

  const cardClass = isDark ? 'bg-black/40 border border-slate-800' : 'bg-white border border-slate-200 shadow-sm'
  ...
  const renderMatchCard = match => {
    ...
    return (
      <div
        key={match.id}
        className={`flex flex-col gap-2 p-3 rounded-lg border my-2 ${isDark ? 'border-slate-700 bg-slate-900/80' : 'border-slate-200 bg-white'}`}
      >
        {/* Date / Status Header */}
        ...
        <div className='flex justify-between items-center text-xs'>
          <div className='flex items-center gap-2 overflow-hidden'>
            ...
            <div className='w-4 h-4 rounded-full bg-slate-700 shrink-0'>
              <img
                src={resolveAnyClubLogo(match.homeTeam?.name)}
                alt=''
        ...
        <div className='flex justify-between items-center text-xs'>
          <div className='flex items-center gap-2 overflow-hidden'>
            <div className='w-4 h-4 rounded-full bg-slate-700 shrink-0'>
              <img
                src={resolveAnyClubLogo(match.awayTeam?.name)}
                alt=''
                className='w-full h-full object-contain'
                onError={e => (e.target.style.display = 'none')}
              />
  ...
  const renderStageColumn = (stageName, label) => {
    ...
    return (
      <div className='min-w-[200px] flex flex-col h-full mx-2'>
        <div
          className={`text-center py-2 mb-4 rounded border ${isDark ? 'bg-slate-900 border-slate-700 text-slate-400' : 'bg-slate-100 border-slate-200 text-slate-600'} text-[10px] font-bold uppercase tracking-widest`}
        >
          {label}
        </div>
    ...
    <div className='space-y-8 w-full animate-in fade-in duration-500'>
      ...
      <section>
        ...
        <div className={`${cardClass} rounded-2xl p-6 overflow-x-auto custom-scrollbar`}>
          {loadingMatches ? (
            <div className='h-64 flex items-center justify-center text-slate-500 text-sm'>Loading Bracket Data...</div>
          ) : (
              ...
              <tbody>
                {loadingMatches ? (
                  ...
                  bracketMatches.map(m => (
                    <tr
                      key={m.id}
                      className={`transition-colors ${isDark ? 'hover:bg-slate-800/30' : 'hover:bg-slate-50'}`}
                    >
                      ...
                      <td className={cellClass}>
                        {m.utcDate ? (
                          <>
                            {new Date(m.utcDate).toLocaleDateString('id-ID', {
                              day: 'numeric',
                              month: 'short',
                              year: 'numeric'
                            })}
